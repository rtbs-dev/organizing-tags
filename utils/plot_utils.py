...
def save_fig(fig, file_name, fmt=None, dpi=600, tight=True, **save_kws):
    ...
    # save figure
    if tight:
        fig.savefig(tmp_name, dpi=dpi, bbox_inches='tight', **save_kws)
    else:
        fig.savefig(tmp_name, dpi=dpi, **save_kws)
...
def plot_or_save(dir, args, fname, fig=None, **save_kws):
    """ show or save plots in an experiment run.
    REQUIRES `logging`!"""
    if fig is None:
        fig = plt.gcf()
    if args.save:
